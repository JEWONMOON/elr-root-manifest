...
# 6. CONDITIONAL REASONING ENGINE - Agnes의 핵심 추천
class ConditionalReasoningEngine:
    """조건문 추론 전용 엔진 - 논리적 조건문의 정확한 처리"""
    
    def __init__(self):
        self.conditional_rules = {}
        self.truth_table_cache = {}
        self.possible_worlds_cache = {}
        
    def evaluate_conditional(self, antecedent: str, consequent: str, 
                           knowledge_base: Dict[str, Any]) -> Dict[str, Any]:
        """조건문 A→B의 진리값 엄밀 평가"""
        
        # 1. 전건이 가능한 세계들 식별
        possible_worlds = self._get_satisfiable_worlds(knowledge_base)
        antecedent_worlds = [w for w in possible_worlds if self._satisfies(w, antecedent)]
        
        # 2. 전건이 불가능하면 vacuously true
        if not antecedent_worlds:
            return {
                "truth_value": True,
                "type": "vacuously_true",
                "reason": f"'{antecedent}' is impossible in all worlds",
                "confidence": 1.0
            }
        
        # 3. 전건이 참인 모든 세계에서 후건도 참인지 확인
        violations = []
        confirmations = []
        
        for world in antecedent_worlds:
            if self._satisfies(world, consequent):
                confirmations.append(world)
            else:
                violations.append(world)
        
        is_true = len(violations) == 0
        confidence = len(confirmations) / len(antecedent_worlds) if antecedent_worlds else 0
        
        return {
            "truth_value": is_true,
            "type": "material_conditional" if is_true else "false_conditional",
            "antecedent_worlds": len(antecedent_worlds),
            "confirmations": len(confirmations),
            "violations": violations[:3],  # 처음 3개만
            "confidence": confidence,
            "logical_form": f"∀x({antecedent}(x) → {consequent}(x))"
        }
    
    def _get_satisfiable_worlds(self, kb: Dict[str, Any]) -> List[Dict[str, bool]]:
        """지식베이스와 일관된 가능 세계들 생성"""
        
        # 캐싱으로 성능 최적화
        kb_hash = str(sorted(kb.items()))
        if kb_hash in self.possible_worlds_cache:
            return self.possible_worlds_cache[kb_hash]
        
        # 변수 추출
        variables = kb.get('variables', ['A', 'B', 'C', 'D'])
        ...
        for values in itertools.product([True, False], repeat=len(variables)):
            ...
            # 제약조건 검사
            if self._check_constraints(world, constraints):
                worlds.append(world)
        
        self.possible_worlds_cache[kb_hash] = worlds
        return worlds
    
    def _satisfies(self, world: Dict[str, bool], formula: str) -> bool:
        """특정 세계에서 공식이 참인지 평가"""
        
        # 간단한 명제 논리 평가기
        # 실제로는 더 복잡한 파서가 필요하지만, 데모용으로 단순화
        
        # 부정 처리
        if formula.startswith("¬") or formula.startswith("~"):
            inner = formula[1:].strip()
            return not self._satisfies(world, inner)
        
        # 연언 처리 (and)
        if "∧" in formula or " and " in formula:
            parts = formula.replace("∧", " and ").split(" and ")
            return all(self._satisfies(world, p.strip()) for p in parts)
        
        # 선언 처리 (or)
        if "∨" in formula or " or " in formula:
            parts = formula.replace("∨", " or ").split(" or ")
            return any(self._satisfies(world, p.strip()) for p in parts)
        ...
        if "→" in formula:
            ...
            if len(parts) == 2:
                ant = parts[0].strip()
                con = parts[1].strip()
                return not self._satisfies(world, ant) or self._satisfies(world, con)
    ...
    def _check_constraints(self, world: Dict[str, bool], constraints: List[Dict[str, Any]]) -> bool:
        """제약조건 검사"""
        
        for constraint in constraints:
            if constraint['type'] == 'implication':
                # A → B 형태
                if world.get(constraint['antecedent']) and not world.get(constraint['consequent']):
                    return False
            elif constraint['type'] == 'exclusive':
                # A ⊕ B (둘 중 하나만)
                vars = constraint['variables']
                true_count = sum(world.get(v, False) for v in vars)
                if true_count != 1:
                    return False
            elif constraint['type'] == 'at_least_one':
                # A ∨ B ∨ C (최소 하나)
                vars = constraint['variables']
                if not any(world.get(v, False) for v in vars):
                    return False
        
        return True


# 7. BIAS DETECTION SYSTEM - 구체적 구현
class BiasDetectionSystem:
    ...
    def __init__(self):
        self.bias_patterns = {
            ...
            'pattern_matching_fallacy': self._detect_pattern_matching_fallacy,
            'availability_heuristic': self._detect_availability_heuristic
        }
        self.reasoning_history = deque(maxlen=100)
        
    def detect_biases(self, reasoning_trace: Dict[str, Any]) -> Dict[str, Any]:
        """모든 편향 감지 실행"""
        
        detected_biases = {}
        
        for bias_name, detection_func in self.bias_patterns.items():
            result = detection_func(reasoning_trace)
            if result.get('detected'):
                detected_biases[bias_name] = result
        
        # 전체 편향 심각도 계산
        overall_severity = self._calculate_overall_severity(detected_biases)
        
        return {
            'detected_biases': detected_biases,
            'bias_count': len(detected_biases),
            'overall_severity': overall_severity,
            'recommendations': self._generate_debiasing_recommendations(detected_biases)
        }
    
    def _detect_complexity_bias(self, reasoning_trace: Dict[str, Any]) -> Dict[str, Any]:
        """형식적 복잡성 편향 감지"""
        
        option_evaluations = reasoning_trace.get('option_evaluations', [])
        ...
        for opt in option_evaluations:
            ...
            complexity = {
                'length': len(text),
                'clause_count': text.count('면') + text.count('라면') + text.count('→'),
                'negation_count': text.count('않') + text.count('아니') + text.count('¬'),
                'logical_operators': text.count('∧') + text.count('∨') + text.count('⊕')
            }
        ...
        if len(complexity_scores) > 2:
            correlation = np.corrcoef(complexity_scores, evaluation_scores)[0, 1]
            ...
            if abs(correlation) > 0.6:
                return {
                    ...
                    "evidence": f"복잡성-점수 상관계수: {correlation:.3f}",
                    "recommendation": "논리적 타당성만으로 재평가 필요"
                }
        
        return {"detected": False}
    
    def _detect_confirmation_bias(self, reasoning_trace: Dict[str, Any]) -> Dict[str, Any]:
        """확증편향 감지"""
        
        # 가설을 지지하는 증거 vs 반박하는 증거의 비율 확인
        supporting_evidence = reasoning_trace.get('supporting_evidence', [])
        contradicting_evidence = reasoning_trace.get('contradicting_evidence', [])
        
        if not supporting_evidence and not contradicting_evidence:
            return {"detected": False}
        
        support_ratio = len(supporting_evidence) / max(1, len(supporting_evidence) + len(contradicting_evidence))
